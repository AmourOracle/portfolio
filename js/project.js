document.addEventListener('DOMContentLoaded', () => {
    ...
    function setActiveItem(index, smooth = true) {
        // MOD: (Req 3) 確保索引在 0 和 (長度-1) 之間
        if (index < 0 || index >= allProjectItems.length || allProjectItems[index].classList.contains('hide')) {
            
            // 如果索引無效 (例如篩選後無項目)，則恢復預設
            // (我們也檢查 .hide，確保不會選中隱藏項)
    ...
    fetch('./data/projects.json')
        ...
        .then(projects => {
            ...
            // MOD: (Req 3) 預設啟用第三個項目 (索引 2)
            if (allProjectItems.length > 2) {
                setActiveItem(2, false); // 立即跳轉，不使用平滑滾動
            } else if (allProjectItems.length > 0) {
                setActiveItem(0, false); // 如果項目總數少於 3，則啟用第一個
            }
        })
        .catch(error => {
